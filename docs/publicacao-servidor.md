...
## Particularidades do servidor do CAMPS (<servidor>)
...
- O perfil ativo é `C:\Users\<conta-local>.<host-antigo>` (nome antigo da
  máquina). Atalho colocado em `C:\Users\<conta-local>` não aparece para
  ninguém.
